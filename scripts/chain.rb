...
class ChainEdge
	...
	def initialize(_chains,do_sort=false)
		@chains = (do_sort)?(_chains.sort {|x,y| x<=>y}):(_chains)
		reset_counts
	end
...
class CSChainEdge < ChainEdge
	def weight
		...
		@counts.each_with_index.inject(0) do |psum,(count,i)| 	
			...
			when 0
				(@chains[0].total_size + @chains[1].total_size) 
			when 1 
				(@chains[1].total_size + @chains[2].total_size) 
			when 2
				(@chains[0].total_size + @chains[1].total_size + @chains[2].total_size) 
			when 3
				(@chains[2].total_size + @chains[3].total_size) 
			when 4
				(@chains[1].total_size + @chains[2].total_size + @chains[3].total_size) 
			when 5
				(@chains[0].total_size + @chains[1].total_size + @chains[2].total_size + @chains[3].total_size) 
			end
		end
	end
