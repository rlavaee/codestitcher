...
class HeapNode
  ...
  def heapify_down
    _children = @children.reject{|child| child.nil? || child.data.nil?}
    begin
      if(!_children.empty? && self < _children.max)
        _children.max.swap_with_parent
        heapify_down
      end
    rescue ArgumentError => e
      puts e
      _children.each {|c| puts c.to_s}
      raise
    end
  end
